...
UI.prototype = Object.create(PIXI.DisplayObjectContainer.prototype), UI.prototype.constructor = UI, UI.prototype.build = function() {
	...
	var s = this.affinity.dominant[i],
		o = affinityLevels[s][t + 1];
	...
	if ("Dominant" !== this.affinity.dominant[i]) {
		...
		for (var e in this.affinity.dominant) e != i && (this.affinity.dominant[e] = "NonDominant")
...
function TechWeb(i, t) {
	...
	function n(i, t) {
		...
		var o = [0, 0, 6, 0],
			...
			a = [39, 0, 31, 0],
			s = [39, 33, 0, 0],
			h = PIXI.Sprite.fromFrame("tech-disabled.png"),
			l = PIXI.Sprite.fromFrame("tech-disabled-small.png"),
			r = PIXI.Sprite.fromFrame("tech-enabled.png"),
			d = PIXI.Sprite.fromFrame("tech-enabled-small.png"),
			p = PIXI.Sprite.fromFrame("tech-selected.png"),
			c = PIXI.Sprite.fromFrame("tech-selected-small.png"),
			f = PIXI.Sprite.fromFrame("leaf-title.png"),
			v = PIXI.Sprite.fromFrame("node-icons.png"),
			w = PIXI.Sprite.fromFrame("node-title.png");
		...
		var A = 40,
			F = 30,
			z = 30;
		if ("undefined" != typeof t.Affinity)
			for (var L in t.Affinity) {
		...
		if ("undefined" != typeof t.Buildings)
			for (var Y in t.Buildings) {
		...
		if ("undefined" != typeof t["Tile Improvement"])
			for (var B in t["Tile Improvement"]) {
		...
		if ("undefined" != typeof t.Effect)
			for (var _ in t.Effect) {
		...
		return Z.build(i, I[i], t, "tech", S), Z.position.x = e.position.x + 40, Z.position.y = e.position.y + 40, J.addChild(Z), P.tooltip = J.children.length - 1, P.mouseover = function() {
			...
			I[this.name] = 0;
			var i = x.indexOf(this.name); - 1 != i && (x = x.slice(0, i))
		}, e.turnOn = function() {
			...
			if (I[this.name] = .5, this.isOn = !1, this.isEnabled = !0, i) {
				var t = x.indexOf(this.name); - 1 != t && (x = x.slice(0, t))
			}
		}, e
	}
	...
	function h(i) {
		if (d(i), techPtr = _.getChildAt(y[i]), techPtr.getChildAt(techPtr.offset + 2).alpha = 1, techPtr.getChildAt(techPtr.offset + 1).alpha = 0, techPtr.getChildAt(techPtr.offset).alpha = 0, techPtr.getChildAt(techPtr.offset + 4).alpha = M, techPtr.getChildAt(techPtr.offset + 4).alphaWas = M, techPtr.isOn = !0, techPtr.isEnabled = !1, I[i] = 1, C[i] = !0, "Habitation" !== i && (P += techs[i].Science, "undefined" != typeof techs[i].Affinity))
			for (var t in affinityData) "undefined" != typeof techs[i].Affinity[t] && tw.ui.updateAffinity(t, techs[i].Affinity[t]);
	...
	function p(i, t, e, o) {
		...
		if ("undefined" == typeof v[n]) {
			var e = e,
				a = _.children[y[i]].position.x,
				s = _.children[y[i]].position.y,
				h = _.children[y[t]].position.x,
				l = _.children[y[t]].position.y;
			c(n, 4, e, o, a, s, h, l)
		}
	}
	...
	function f() {
		...
		for (var e in y) {
			...
			if ("undefined" != typeof o.techData && "undefined" != typeof o.techData.Enables)
				for (var n in o.techData.Enables) 1 === I[e] && 1 === I[n] ? (i = w.selected.color, t = w.selected.alpha) : .5 === I[e] && 1 === I[n] || 1 === I[e] && .5 === I[n] ? (i = w.enabled.color, t = w.enabled.alpha) : (i = w.disabled.color, t = w.disabled.alpha), p(e, n, i, t)
		}
	}
	...
	var v = [],
		...
		E = .5,
		z = !0,
		L = new PIXI.Stage(0, z),
		T = new PIXI.CanvasRenderer(O, X, document.getElementById("techWebCanvas"), !1);
	...
	var k = PIXI.Texture.fromImage(assetsPath + "background.jpg"),
		j = new PIXI.Sprite(k);
	...
	var N = PIXI.Texture.fromImage(assetsPath + "loading.jpg"),
		U = new PIXI.Sprite(N);
	U.anchor.x = .5, U.anchor.y = .5, R.addChild(U);
	var B = [assetsPath + "sprites.json"],
		G = new PIXI.AssetLoader(B);
	...
	var Q = PIXI.Texture.fromImage("img/rings.png"),
		V = new PIXI.Sprite(Q);
